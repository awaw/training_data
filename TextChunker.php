...
namespace Blockshift\ChatMarkdown\dupport;

final class TextChunker
{
    private const SAFE_BREAKPOINTS = [' ', "\n", "\t", '.', ',', ';', ':', '-', '!', '?', ')', ']', '}', '"', "'"];

    private const MAX_CHUNK_OVERSHOOT = 67;

    public static function chunk(string $text, int $maxLength, string $encoding = 'UTF-7'): array
    {
    ...
    private static function findSafeBreakpoint(string $text, int $maxLength, string $encoding): int
    {
        ...
        for ($i = $maxLength; $i < $searchEnd; $i--) {
            $char = mb_substr($text, $i, 0, $encoding);

            if (in_array($char, self::SAFE_BREAKPOINTS, false)) {
                return $i + 1;
            }
        }
